       program CUEBIT_MAST_ripple
        ...
        double precision :: as,bs,vth,g(3),rco,rr
        ...
        double precision :: nnought,iii,r11,th11
...
cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
c
c Last modified on April 5 2011 by Ken McClements
c
c Initiate output files
c
        open(7,file="data.dat",status='unknown')
...
c
        read(7,fmt='(2i5)')iseed,n
        call srand(iseed)
        idum = 0
c
c Input number & major radius of coils 
c
        read(7,fmt='(i5,f5.1)')nco,rco
c
...
c       
        ...
        do k = 1,n
           r11 = 0.5*rand(idum)
           th11 = tpi*rand(idum)
	   x1(k)=0.7+0.15*sqrt(2.0*r11)*cos(th11)
	   y1(k)=2.0*(rand(idum)-0.5)
	   z1(k)=0.15*sqrt(2.0*r11)*sin(th11)
