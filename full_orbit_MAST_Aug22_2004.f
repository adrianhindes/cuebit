...
c
      ...
      do i=1,nr
         do j=1,nz
            ...
            goto 30
 10         dz=zgrid(j+1)-zgrid(j)
            br(i,j)=(psi(i,j+1)-psi(i,j))/dz/2.d0/pi/rgrid(i)
            goto 30
 20         dz=zgrid(j)-zgrid(j-1)
...
c
      do j=1,nz
         do i=1,nr
            ...
            goto 60
 40         dr=rgrid(i+1)-rgrid(i)
            bz(i,j)=-(psi(i+1,j)-psi(i,j))/dr/2.d0/pi/rgrid(i)
            goto 60
 50         dr=rgrid(i)-rgrid(i-1)
...
c time steps
c
	write(6,fmt='("time step, no. of steps & fraction of steps output")')
	read(5,*)dt,nt,fi
...
c
	...
	phid=phi2*180.d0/pi
        ...
        i2=(i/i1)*i1
        if(i2.eq.i)write(8,fmt='(1p,6d11.3)')t,r2,phid,z2,psi2,errpphi
	x1=x2
	y1=y2
	z1=z2
	r1=r2
